#!/usr/bin/env python3
"""
ISAAC AI – Menu‑driven control center for your offline AI assistant.
"""
...
THREADS = 4
GPU_LAYERS = 0
LEARNER_BIN = "./learner"   # adjust if needed
# ----------------------------

# -------- ASCII BANNER --------
BANNER = """
┌─────────────────────────────────────────────────────────────────┐
│                     ISAAC AI – Complete System                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐       │
│  │   LEARNER    │    │    ASK       │    │   MANAGER    │       │
...
│  │              │    │              │    │              │       │
│  │ • Scans      │    │ • TF‑IDF     │    │ • Starts     │       │
│  │   storage    │    │   retrieval  │    │   server     │       │
│  │ • Chunks     │    │ • Calls      │    │ • Starts     │       │
│  │   documents  │    │   llama‑     │    │   learner    │       │
│  │ • Builds     │    │   server     │    │ • Launches   │       │
│  │   vectors    │    │ • Displays   │    │   ask        │       │
│  │              │    │   answers    │    │              │       │
│  └──────────────┘    └──────────────┘    └──────────────┘       │
│         │                    │                    │              │
│         └────────────────────┼────────────────────┘              │
...
def run_learner():
    if not Path(LEARNER_BIN).exists():
        print(f"❌ Learner binary '{LEARNER_BIN}' not found.")
        return
    print("📚 Scanning storage and building vectors (this may take a while)...")
    print("   (Use Ctrl+C to stop, but be careful not to corrupt the index.)")
    cmd = ["nice", "-n", "19", LEARNER_BIN]
    try:
        subprocess.run(cmd, check=True)
        print("✅ Learning complete.")
    except subprocess.CalledProcessError as e:
        print(f"❌ Learner failed with code {e.returncode}.")
...
def main_menu():
    ...
    while True:
        ...
        print("  5. Status")
        print("  6. Run learner (scan storage & build vectors)")
        print("  7. Exit")
